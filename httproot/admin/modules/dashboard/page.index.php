...
?>
	<!-- Call Status Added by JAT 6/2/2015 -->
	<script>
		$callStatusURI = '/callstatus/status.php';
		var auto_refresh = setInterval(
			function()
			{
				var status = $("#callStatus").val();
				$('#callStatus').load($callStatusURI, {status:status});
			}, 5000);
		$(document).ready(function () {
			$('#callStatus').load($callStatusURI);
		});
	</script>
	<div id="callStatus" style="float:left; margin-left: 50px; width:40%;" ></div>
	<!-- End Call Status -->
<?php
